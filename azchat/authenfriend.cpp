    ...
    , ui(new Ui::AuthenFriend)
{
    ...
    connect(ui->more_lb, &ClickedOnceLB::clicked, this, &AuthenFriend::ShowMoreLabel);
    InitTipLbs();
    //链接输入标签回车事件
    ...
    connect(ui->tip_lb, &ClickedOnceLB::clicked, this, &AuthenFriend::SlotAddFirendLabelByClickTip);
...
void AuthenFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        connect(lb, &ClickedLable::clicked, this, &AuthenFriend::SlotChangeFriendLabelByTip);
...
void AuthenFriend::AddTipLbs(ClickedLable *lb, QPoint cur_point, QPoint &next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
void AuthenFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickedLbState state)
{
...
void AuthenFriend::SlotApplySure()
{
    ...
    jsobj["back"] = back_name;
    QJsonDocument doc(jsobj);
    QByteArray data =  doc.toJson(QJsonDocument::Compact);

    emit TcpMgr::Get_instance()->sig_send_data(Reqids::ID_AUTH_FRIEND_REQ,data);
    this->hide();
    this->deleteLater();
}
...
void AuthenFriend::addLabel(QString name)
{
    ...
    connect(tmplabel, &FriendLb::sig_close, this, &AuthenFriend::SlotRemoveFriendLabel);
